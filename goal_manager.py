...
class GoalManager:
    ...
    async def set_daily_goal(self, user_id: int, goal: int):
        ...
        cursor.execute(
            "INSERT OR REPLACE INTO daily_goals (discord_id, goal, last_updated, penalties, streak, last_check) VALUES (?, ?, ?, 0, 0, ?)",
    ...
    async def check_daily_progress(self, user_id: int):
        ...
        cursor.execute("SELECT goal FROM daily_goals WHERE discord_id = ?", (user_id,))
        ...
        return True, {
            'goal': goal_data['goal'],
            'solved': solved_today,
            'remaining': max(0, goal_data['goal'] - solved_today)
        }
    ...
    async def apply_daily_penalties(self):
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT discord_id, goal FROM daily_goals")
    ...
    async def get_user_stats(self, user_id: int):
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT goal, penalties, streak, last_penalty, last_check FROM daily_goals WHERE discord_id = ?", (user_id,))
        goal_data = cursor.fetchone()
        if not goal_data:
            return False, "No daily goal set"
